...
export default function BlockList() {
  ...
  const load = async () => {
    ...
    try {
      ...
      console.log("[BlockList] API data:", data);
  ...
  // 첫 렌더링 시(그리고 myId 유효성 바뀌면) 목록 로드
  useEffect(() => {
    load();
  }, [isValidMyId]);
  ...
  const items = Array.isArray(blocks)
    ? blocks
    : Array.isArray(blocks?.blocklist)
      ? blocks.blocklist
      : [];
